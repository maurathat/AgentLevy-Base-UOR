...
def test_currency_defaults_to_usdc():
    """USDC is the default settlement currency on Base (per Coinbase x402 spec)."""
    buyer = Keypair.from_seed(TEST_SEED_BUYER)
    seller = Keypair.from_seed(TEST_SEED_SELLER)
    spec = _build_spec(buyer, seller)
    assert spec.currency == CURRENCY_USDC


def test_eth_currency_accepted():
    buyer = Keypair.from_seed(TEST_SEED_BUYER)
    seller = Keypair.from_seed(TEST_SEED_SELLER)
    spec = _build_spec(buyer, seller, currency=CURRENCY_ETH)
    assert spec.currency == CURRENCY_ETH
...
def test_invalid_currency_rejected():
    ...
    with pytest.raises(ValueError, match="currency"):
        _build_spec(buyer, seller, currency="RLUSD")  # XRPL-side currency, not allowed here
