...
#: Allowed settlement-currency values. USDC is the default for the Base
#: settlement chain (Coinbase x402 spec settles in USDC on Base). ETH
#: remains permitted as a fallback for environments without USDC. The
#: parallel sibling implementation (AgentLevy-XRPL-UOR) uses RLUSD as
#: its default; protocol primitives are chain-agnostic — currency just
#: travels with the chain.
CURRENCY_USDC = "USDC"
CURRENCY_ETH = "ETH"
ALLOWED_CURRENCIES = frozenset({CURRENCY_USDC, CURRENCY_ETH})
...
class TaskSpec(BaseModel):
    ...
    price_drops: int = Field(
        ...,
        ge=1,
        description="Amount in the chain's smallest unit (wei for ETH; "
                    "USDC base units = 6 decimals on Base).",
    )
    chain: str = Field(default="base", description="Settlement chain identifier.")
    currency: str = Field(
        default=CURRENCY_USDC,
        description="Settlement currency. Defaults to USDC per Coinbase "
                    "x402 spec; ETH also permitted. See ALLOWED_CURRENCIES.",
    )
